...
function NaverConnectModal(props: {
 ...
 <button onClick={onClose} className="text-[#8B95A1] p-1"><X size={20} strokeWidth={2.5} /></button>
 </div>
...
function GoogleConnectModal(props: {
 ...
 {r.rating && <p className="text-[11px] text-[#F5A623] inline-flex items-center gap-0.5"><Star size={11} className="fill-current" />{r.rating} ({r.userRatingsTotal?.toLocaleString()}건)</p>}
 </button>
 ))}
 </div>
 )}
 ...
 {preview.rating > 0 && <span className="ml-auto text-xs text-[#F5A623] inline-flex items-center gap-0.5"><Star size={12} className="fill-current" />{preview.rating} ({preview.reviewCount?.toLocaleString()}건)</span>}
 </div>
 <p className="text-sm font-bold text-[#191F28]">{preview.name}</p>
